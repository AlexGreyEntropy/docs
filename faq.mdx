...
<AccordionGroup>
  <Accordion title="Is Thaler self-custodial?" defaultOpen>
    ...
    at creation. See [Custody and policy](/overview/custody-and-policy).
  </Accordion>
  ...
  <Accordion title="Where are my funds while the vault is running?">
    Inside the Squads smart account that you co-own. The smart account holds the deposit, the
    liquid staking token, the lending position, and the perpetual margin. The protocol never
    has custody of your funds.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="Is the principal really protected?">
    ...
    variance. The protection does not extend to a smart-contract failure of a third-party
    venue, a loss of user keys, or a close inside the penalty schedule. See
...
<AccordionGroup>
  ...
  <Accordion title="Can I deposit in USDC instead of SOL?">
    ...
    USDC required are shown on screen before you sign.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Can I close before the perpetual hedge settles?">
    The close transaction settles the hedge as part of the same operation. You do not need to
    close the hedge separately or wait for an off-chain confirmation.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Is the headline APY the gross or net rate?">
    ...
    service fee is taken. You do not need to subtract anything to compare across tiers.
  </Accordion>
  <Accordion title="Why does the simulator show different ROI for shorter windows?">
    ...
    holding window at an 11.75 % APY produces around 0.92 % realised ROI; the same band on a
    365-day holding window produces the full 11.75 %. The numbers stay internally consistent
    with the headline APY.
  </Accordion>
</AccordionGroup>
